    ...
    try {
        ...
        Libro.deleteOne({isbn: req.body.isbn})
            ...
            .catch(err => {
                ...
                res.status(500).send({
                    msg: "ER",
                    info: "Ha ocurrido un error en la eliminacion del dato, intente mas tarde"
                });
            });
    } catch (err) {
        ...
        Libro.updateOne({isbn: req.body.isbn}, {
            ...
            .then((rta) => {
                if (rta.modifiedCount < 1) {
                    res.status(404).send({msg: "OK", info: "El libro que digitaste no existe"});
                    return;
                }
                res.send({msg: "OK", info: "El libro ha sido actualizado exitosamente"});
            })
            .catch(err => {
                console.log("ER: " + err);
                res.status(500).send({msg: "ER", info: "Informacion no disponible, por favor intente mas tarde"});
            })
    } catch (err) {
